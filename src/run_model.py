...
patience = config['parameters'].get('patience', 10)
...
for subdir in sorted(os.listdir(h5_dir)):
    ...
    for taste_ind, taste_spikes in enumerate(spike_array):
       ...
       # stim_input[stim_time_val // bin_size, :] = 1 # what I had-- changed to match abu; double check idx'ing
        ...
        plt.suptitle("RNN Input")
        plt.savefig(os.path.join(plots_dir, f'inputs_taste_{taste_ind}.png'))
        plt.close()
       ...
       #     pred_firing = pred_flat.reshape((*pred_firing.shape[:2], -1))
        ...
        fig.savefig(os.path.join(plots_dir, f'loss_taste_{taste_ind}.png'))
        plt.close()
            # --------------------------
    ...
    print(f"✅ Saved predicted firing rates for {dataset_name}")
